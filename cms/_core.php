	...
	static function formField($type,$id,$value="",$title="",$no_error=false){
		...
		switch ($type[0]){
			...
			case "bool":
				...
				list($true,$false) = $type[1] ? explode(",",$type[1])
											   : array(1,0);
											   																							
				$tmp 		= explode("-",$id);
				$table_info = sql::getColumns(self::$PAGE->TABLE);
				
				if($value === $false || (empty($value) && $table_info[$tmp[0]]['Default'] == $false)){	
...
class page {
	...
	// echo '<pre>'; var_dump($_POST); echo '</pre>'; 
		foreach($_POST as $key => $value){
			// echo '<pre>'; var_dump($key); var_dump($value); echo '</pre>';
			$tmp = explode("_",$key);			
			$num = $tmp[1];
	 		// if i place var_dump(); here the script will run instead of giving a 502 error
// die();
			// Ricardo commented this for out because it's causing errors
// var_dump();
			...
			foreach($entries as $field => $value)				
				if(in_array($field,$this->SHOWN_FIELDS) && !is_numeric($field)){
					...
					if($field_type == "img"){
						...
						$value = ($value == $true) ? '<img src="images/tick.png" id="bool_'.$entries['id'].'" data-true="'.$true.'" data-false="'.$false.'"  class="bool_click" rel="'.$field.'|'.$entries['id'].'" />'
											       : '<img src="images/cross.png" id="bool_'.$entries['id'].'" data-true="'.$true.'" data-false="'.$false.'" class="bool_click" rel="'.$field.'|'.$entries['id'].'" />';
